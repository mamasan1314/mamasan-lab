...
| `instagram_business_manage_comments` | **寫入完整可用**（建立、回覆、刪除皆成功）；**讀取全擋**（標準存取） |
...
1. 先關 C 類（廣告與企業管理），完全不相干，零風險。**但目前關不掉，見下一節。**
2. 再關 B 類（Facebook Login 路線）—— 但**如果將來要連粉專走那條路，會需要重開**。
3. A 類與 D 類**先不要動**。

### 為什麼「廣告與企業管理」關不掉（2026-09-04）

`ads_management` 那幾項的「操作」選單裡，只有「新增到應用程式檢閱」可以點，
**「使用案例的必要項目」是灰的** —— 那不是一個可以點的動作，是一個**狀態標示**：
這項權限是某個使用案例的必要組成，因此不能在權限頁上單獨移除。

線索就在同一列：那幾項都標著「在 2 個使用案例中找到」「在 3 個使用案例中找到」。
新版 Dashboard 以**使用案例（Use Case）**為單位打包權限，移除的閘門開在使用案例那一層。

**要移除就去「使用案例」那一頁，把不需要的整個使用案例移掉**，不是在權限頁上找開關。

**但先不要動。**動之前必須確認：那個使用案例裡有沒有夾帶我們正在用的五項
（`instagram_business_*`）。整個移掉的話會連帶拔掉核心權限，而依上面那條結論，
三支權杖會**立刻**失去能力，發布與洞察會同時停擺。
點那個「在 N 個使用案例中找到」連結可以看到是哪幾個，先看清楚再決定。

**這不是為了推銷廣告。**那幾項都是 0 次呼叫、標準存取，而且我們的權杖是
Instagram Login 型（`IGA` 前綴），廣告權限屬於 Facebook 圖形 API 那一側，
對這支權杖形同惰性。留著不會產生任何費用，也不會有任何東西被啟用 ——
它們只是被使用案例的打包方式綁在清單上。優先度低。
