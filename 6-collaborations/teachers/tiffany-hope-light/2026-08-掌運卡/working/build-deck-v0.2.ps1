...
$scriptPath = Join-Path $ProjectDir 'outputs\掌運卡_教學版_講師草稿_v0.2_逐頁腳本.md'
$outputDir = Join-Path $ProjectDir 'outputs'
$previewDir = Join-Path $ProjectDir 'working\previews-v0.2'
...
function Add-TextBox {
    ...
    # AddTextbox ignores the requested height and collapses to one line; pin the
    # frame after the text is in, otherwise middle-anchored text overflows upward.
...
function Add-DeckChrome {
    ...
    if (-not $IsCover) {
        ...
        $line.Line.Visible = 0
        [void](Add-TextBox -Slide $Slide -Text ('{0:D2}  ·  v0.2 講師草稿' -f $SlideNumber) -Left 720 -Top 505 -Width 186 -Height 20 -FontSize 9 -Color $colors.muted -Alignment 3)
    }
}
...
function Get-FittedFontSize {
    ...
    $paragraphs = @($Text -split "`r|`n" | Where-Object { $_ -ne $null })
    if ($paragraphs.Count -eq 0) { return $MaxSize }

    for ($size = $MaxSize; $size -ge $MinSize; $size -= 0.5) {
        $charsPerRow = [Math]::Max(1, [Math]::Floor($BoxWidth / ($size * 1.02)))
        $rows = 0
        foreach ($p in $paragraphs) {
            $len = $p.Length
            if ($len -eq 0) { $rows += 1; continue }
            $rows += [Math]::Max(1, [Math]::Ceiling($len / $charsPerRow))
        }
        $needed = ($rows * $size * 1.42) + ($paragraphs.Count * 7)
        if ($needed -le $BoxHeight) { return $size }
    }
    return $MinSize
}
...
function Add-CoverSlide {
    ...
    [void](Add-TextBox -Slide $Slide -Text '人人可用　自由提問　內在連結' -Left 72 -Top 330 -Width 560 -Height 54 -FontSize 20 -Color $colors.white)
    [void](Add-TextBox -Slide $Slide -Text '♠   ♥   ♦   ♣' -Left 70 -Top 425 -Width 390 -Height 44 -FontSize 28 -Color $colors.paleRose)
    [void](Add-TextBox -Slide $Slide -Text 'v0.2｜講師草稿' -Left 72 -Top 487 -Width 220 -Height 20 -FontSize 10 -Color $colors.paleGold)
}
...
function Add-StatementSlide {
    ...
    $fontSize = Get-FittedFontSize -Text $body -BoxWidth 700 -BoxHeight 176 -MaxSize 29 -MinSize 15
    [void](Add-TextBox -Slide $Slide -Text $body -Left 122 -Top 187 -Width 716 -Height 192 -FontSize $fontSize -Color $colors.ink -Bold $true -Alignment 2 -VerticalAnchor 3)
...
function Add-SuitOverviewSlide {
    ...
    $items = @(
        @{ symbol = '♠'; name = '黑桃'; meaning = '意志・力量・界線'; color = $colors.ink; fill = $colors.paper },
        @{ symbol = '♥'; name = '紅心'; meaning = '情感・關係・內心需求'; color = $colors.rose; fill = $colors.paleRose },
        @{ symbol = '♦'; name = '紅磚'; meaning = '金錢・事業・資源・現實'; color = $colors.gold; fill = $colors.paleGold },
        @{ symbol = '♣'; name = '黑梅'; meaning = '消耗・混亂・比原來狀態差'; color = $colors.sage; fill = $colors.paleSage }
    )
...
function Add-NumberSlide {
    ...
    $body = Convert-ScreenText $Data.screen
    $fontSize = Get-FittedFontSize -Text $body -BoxWidth 486 -BoxHeight 264 -MaxSize 25 -MinSize 14
    [void](Add-TextBox -Slide $Slide -Text $body -Left 370 -Top 139 -Width 500 -Height 270 -FontSize $fontSize -Color $colors.ink -Bold $false -VerticalAnchor 3)
}
...
function Add-StandardSlide {
    ...
    $fontSize = Get-FittedFontSize -Text $body -BoxWidth 756 -BoxHeight 226 -MaxSize 25 -MinSize 13
    [void](Add-TextBox -Slide $Slide -Text $body -Left 94 -Top 198 -Width 772 -Height 236 -FontSize $fontSize -Color $colors.ink -VerticalAnchor 3)
}
...
if ($slideData.Count -ne 43) {
    throw ('Expected 43 slide sections, found {0}.' -f $slideData.Count)
}
...
try {
    ...
    foreach ($data in $slideData) {
        ...
        elseif ($data.number -eq 14) {
            Add-SuitOverviewSlide -Slide $slide -Data $data
        }
        elseif ($data.number -ge 21 -and $data.number -le 33) {
            Add-NumberSlide -Slide $slide -Data $data
        }
        elseif ($data.number -in @(4, 7, 11, 43)) {
