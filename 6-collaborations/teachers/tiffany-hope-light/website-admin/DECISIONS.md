...
- 決定日期：2026-09-08（Asia/Taipei）
- 決定者：Darren
- 狀態：**已定案**（通則保留）。**蠟燭 21 顆組不再適用，見 D-007（2026-09-24）。**
...
## D-007｜蠟燭 21 顆組：人工回應，不做 WooCommerce／LINE 自動化；商業條件由 mamasan 決定

- 決定日期：2026-09-24（Asia/Taipei）
- 決定者：Tiffany（人工回應，早已決定）；Darren 2026-09-24 轉述並確認
- 狀態：**已定案**

### 決定內容

1. **蠟燭 21 顆組採人工回應。**顧客從 LP 按 LINE 私訊，由老師端人工回覆、收款、配款。
   不做 WooCommerce 結帳、不建 LINE 自動動線、不做 D-006 的後台自助欄位。
2. **老師已把蠟燭全權交給 mamasan。**售價、運費、含運、贈品、下單流程由 mamasan 決定，
   不再回頭找老師確認。`product-facts.md` 裡「待老師確認」的蠟燭項目改由 mamasan 定案。

### 考慮過、不採用

- **WooCommerce × LINE × CRM（`projects/2026-09-candle21-commerce/`，D-006 的第一個採用專案）**：
  不採用。理由是老師很早就決定要人工回應；自動結帳與自助欄位是在解一個她不需要解的問題。
  工程候選（外掛、子主題、階段驗收頁）保留在 Git 作為歷史，不部署。

### 影響

- D-006 對蠟燭 21 **不再適用**（D-006 本身作為通則仍保留）。
- LP 不需要讀後台欄位，所以**靜態 HTML 就是完成品**，不再是「移入 WordPress 前的暫時版」。
- 上線位置：Darren 提議放在官網 `https://hopebox.com.tw/candles`（2026-09-24 查證該網址為 404、未被占用）。
  寫入方式與權限見 `landing-pages/README.md`。
