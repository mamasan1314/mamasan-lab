...
> `Manus/tools/social-publishing/delivery/quick-tunnel.mjs`，
> 並已被引擎主流程使用（`2026-08-31` 與 `2026-09-02` 兩篇貼文的收據皆記
> `mediaDelivery: frozen_buffer_via_cloudflare_quick_tunnel`）。
> 本文件未回頭標記，導致 `2026-09-03` 一次誤判——判定隧道不存在，
> 並開始重寫一份已經存在的東西。
>
> 「已確認不可行的路線」與「Meta 權限層次、權杖類型、resumable upload
> 的結論」這幾節仍然成立，可以繼續引用。
...
> **已完成於 2026-09-01**，見上方作廢說明。以下保留為當時的規格。
